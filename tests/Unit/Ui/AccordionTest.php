...
test('falls back to slot key when label is omitted', function () {
    ...
    // Assert the button text: the key also appears in data-ws-accordion-item,
    // data-ws-accordion-panel and wire:key, so a page-wide assertSee() cannot
    // tell whether the label fallback actually rendered.
    expect(htmlText($html, 'ws-accordion-button'))->toBe('shipping');
});
