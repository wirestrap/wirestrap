@props([
    ...
    'inline' => false,
    'disabled' => false,
    'hasValidation' => config('wirestrap.switch.has_validation', true),
    'hasValidationMessage' => config('wirestrap.switch.has_validation_message', true),
    'defaultAttributes' => config('wirestrap.switch.default_attributes', []),
])
...
@php
    ...
    $wrapperClass = implode(' ', array_filter([
        'ws-form-check ws-form-switch',
        $inline ? 'ws-form-check-inline' : null,
        $reverse ? 'ws-form-check-reverse' : null,
        $attributes->get('class', config('wirestrap.switch.class', '')) ?: null,
    ]));
